import { Paper, Text, Stack, Group, Avatar, Badge, ScrollArea, TextInput, Menu, ActionIcon } from '@mantine/core';
...
function VolunteerList({ volunteers }) {
  ...
  ).filter(v => committeeFilter === 'All' ? true : v.committee === committeeFilter)
    ...
    <Paper shadow="sm" p="md" withBorder>
      <Stack gap="md">
        <Group justify="space-between">
          <Text fw={600} size="lg">Volunteers</Text>
          <Badge size="lg" variant="light">{filteredVolunteers.length} / {volunteers?.length || 0}</Badge>
        </Group>
              ...
              <Paper key={volunteer.id} p="sm" withBorder>
                <Group justify="space-between">
                  <Group gap="sm">
                    ...
                    <div>
                      <Text size="sm" fw={500}>{volunteer.name}</Text>
                      <Text size="xs" c="dimmed">{volunteer.committee}</Text>
                    </div>
                  </Group>
