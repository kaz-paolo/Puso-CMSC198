...
export async function joinEvent(req, res) {
  try {
    ...
    // event and check approval requirement
    const [event] = await sql`
      SELECT approval_required FROM events WHERE id = ${eventId}
    `;
    ...
    // Set status based on approval_required
    ...
    // Insert volunteer with appropriate status
    const registration = await sql`
      INSERT INTO event_volunteers (user_id, event_id, role_id, volunteer_status)
      VALUES (${userId}, ${eventId}, ${roleId || null}, ${volunteerStatus})
  ...
  try {
    ...
    const [stats] = await sql`
      SELECT 
        ...
        COUNT(*) FILTER (WHERE volunteer_status = 'REMOVED') as removed_count,
      FROM event_volunteers
    ...
    const roleStats = await sql`
      SELECT 
        ...
        evr.role_name,
        evr.capacity,
        COUNT(ev.user_id) FILTER (WHERE ev.volunteer_status = 'CONFIRMED') as confirmed_count,
        COUNT(ev.user_id) FILTER (WHERE ev.volunteer_status = 'PENDING') as pending_count
      FROM event_volunteer_roles evr
...
export async function getEventVolunteers(req, res) {
  try {
    ...
    const volunteers = await sql`
      ...
      LEFT JOIN neon_auth."user" au ON ui.auth_user_id = au.id
...
export async function deleteEventResource(req, res) {
  try {
    ...
    //resource and all its sublinks
    //TODO: seperate delete main resource/sublink
    const softDeleted = await sql`
      UPDATE event_resources
      SET 
        deleted_at = CURRENT_TIMESTAMP,
